...
stage('Package') {
    ...
    steps {
        script {
            ...
            sshagent(['slave2']) {
                sh '''
                    scp -o StrictHostKeyChecking=no server-script.sh ec2-user@3.6.160.102:/home/ec2-user
                    ssh -o StrictHostKeyChecking=no ec2-user@3.6.160.102 "bash ~/server-script.sh"
                '''
            }
        }
    }
